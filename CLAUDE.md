...
- **All audio assets are preloaded at boot via `preloadAllAssets()`** (called from `App.tsx`). The splash screen stays visible until `assetsLoaded === true`. After splash hides, every `play(src)` hits the in-memory cache that expo-audio populated via `preload()` → near-instant playback on Android (no decode step). iOS consumes preloaded sources on first player creation, so only the first `play()` of each sound benefits there.
- **Preload requires a real `file://` path on Android — do NOT pass `require(...)` IDs directly to `preload()`.** expo-audio's `preload()` on Android resolves bundled `require()` assets to a raw resource identifier (e.g. `assets_audio_responsestar`) and feeds it to `FileDataSource`, which throws `FileNotFoundException`. The workaround in `preloadAllAssets()`: `Asset.fromModule(src).downloadAsync()` to materialize a `file://` path, fall back to `new File(Paths.cache, ...).copy()` (from `expo-file-system` v55 API) if `localUri` is not `file://`, then call `preload({uri: filePath})`. The resolved URI is stored in `resolvedUris` and reused by `resolveSource()` so `createAudioPlayer` in `play()` consumes the same cached file.
- `play()` exposes `audioLoading: boolean` which is `true` between the call and the moment the new player reports `status.playing === true` (with a 2s safety timeout). UIs use this to gate user input (e.g., `ChoiceButton` `disabled` prop) and avoid the bug where cancelling a still-loading player corrupts the audio engine state.
- `replay()` does `seekTo(0) + play()` on the current player — used by `InstructionButton` to repeat the question audio instantly without re-decoding. After `didJustFinish`, the listener does NOT dispose the player; cleanup happens at the next `play()` call instead, so the player stays alive for potential replay.
